		...
		try {
			...
			if(str.equals("Display")){
				...
				f1.setVisible(true);
					...
					ta.append("hname  hid	  Branchname	  Emailid	   Address	\n");
					while(rs.next())
					{
						ta.append(rs.getString("hname")+"	"+rs.getString("hid")+"	  "+rs.getString("branchname")+"	"+rs.getString("emailid")+"	   "+rs.getString("address")+"	 "+"\n");
					}
					p.add(ta);	
					f1.add(p);
					st.close();
				...
				rs=st.executeQuery(query);
				tx.setText("");
				tx.append("hname   hid	  Branchname	  Emailid	   Address	\n");
				if(rs.next())
					System.out.println(rs.getString("hname"));
					tx.append(rs.getString("hname")+"	     "+rs.getString("hid")+"	     "+rs.getString("branchname")+"	     "+rs.getString("emailid")+" 	    "+rs.getString("address")+"	     "+"\n");
			...
			if(str.equals("del")){
				...
				query="delete FROM hospital  where hid='"+se+"'";
				st=con.createStatement();
			...
			if(str.equals("INS")){
				...
				query="insert into hospital values('"+n+"','"+i+"',"+ag+",'"+r+"','"+b+"')";
				st=con.createStatement();
				st.executeUpdate(query);
				st.close();
			...
			if(str.equals("UP")){
				...
				query="UPDATE hospital SET hname='"+un+"',hid="+id+",branchname='"+uag+"',emailid='"+ay+"',address="+ub+" WHERE hid='"+id+"'";
				st=con.createStatement();
				st.executeUpdate(query);
				st.close();
